  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const ethProvider = new 
ethers.JsonRpcProvider('https://eth-mainnet.g.alchemy.com/v2/cGTUqBrMuqkTR0ZmpVR55i8MCX_eX4kS');
        const solRpcs = 
['https://solana-mainnet.g.alchemy.com/v2/cGTUqBrMuqkTR0ZmpVR55i8MCX_eX4kS'];

        const walletBalances = {};
        for (const address of user.walletAddresses) {
          walletBalances[address] = { status: 'Fetching...' };
          setBalances((prev) => ({ ...prev, [address]: { status: 
'Fetching...' } }));
          try {
            if (ethers.isAddress(address)) {
              const ethBalance = await ethProvider.getBalance(address);
              walletBalances[address] = { eth: 
...
${localStorage.getItem('token')}` }
  ...
  const calculateRiskLevel = () => {
    if (!userData || !userData.portfolio.length) return { level: 'N/A', 
score: 0 };
    let totalRisk = userData.portfolio.reduce((sum, item) => {
      const proto = 
userData?.protocolData[item.protocol.toLowerCase().trim()] || { 
securityScore: 5 };
      console.log(`${item.protocol}: Security Score = 
${proto.securityScore}`); // Debug log
      return sum + (10 - proto.securityScore) * (item.percentage / 100);
    }, 0);
    
    const walletRisk = Object.values(balances).reduce((sum, bal) => 
      sum + ((bal.eth && Number(bal.eth) > 0.1) || (bal.sol && bal.sol > 
10) ? 2 : 0), 0);
    totalRisk += walletRisk;

    return { 
      level: totalRisk > 5 ? 'High Risk' : totalRisk > 2 ? 'Moderate Risk' 
: 'Low Risk', 
...
// Debug log
          const tvlInBillions = proto.tvl >= 1e9 ? (proto.tvl / 
1e9).toFixed(1) : (proto.tvl / 1e9).toFixed(3);
          return (
            <li key={index}>
              {item.protocol}: {item.percentage}% 
              (Security: {proto.securityScore}/10, TVL: ${tvlInBillions}B, 
              Health: {proto.health}, APY: {proto.apy}%) 
              <button onClick={() => handleDeletePortfolioItem(index)} 
className="delete-button">Delete</button>
      ...
      <div className="optimization">
        ...
        <p>Target Risk: {userData.optimization.targetRisk} | Target Yield: 
{userData.optimization.targetYield}%</p>
      </div>
...
${Number(balances[address].eth).toFixed(4)} ETH 
($${((Number(balances[address].eth) * 1800)).toFixed(2)})` : ''}
            {balances[address]?.sol ? ` - 
${Number(balances[address].sol).toFixed(4)} SOL 
($${((Number(balances[address].sol) * 50)).toFixed(2)})` : ''}
            {balances[address]?.error ? ` - ${balances[address].error}` : 
''}
            {balances[address]?.status ? ` - ${balances[address].status}` 
: ''}
            <button onClick={() => handleDeleteWallet(index)} 
className="delete-button">Delete</button>
          </li>
        ))}
      </ul>
...
navigate('/login'); }} className="auth-button 
